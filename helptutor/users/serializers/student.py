...
from helptutor.users.models import User, Student, Tutor, Moderator
...
class StudentUpdateSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        pk_user = self.context['view'].kwargs['pk']
        instance = StudentViewSerializer(Student.objects.get(user=pk_user)).data
        return instance
...
class StudentGoogleCreateSerializer(serializers.Serializer):
    ...
    def create(self, validated_data):
        ...
        user = self.context['user']
        user.send_registration_email('student')
